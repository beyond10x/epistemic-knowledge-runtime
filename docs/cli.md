...
| `store-not-found` | propose, validate, commit, snapshot, explain, head, transactions, ontology | 1 | `--store` names a path that holds no store: nothing, an empty directory, an empty file or a symlink to nothing; nothing is created there. Only `ekr seed` creates a store, and a seed that is refused creates none | check `--store` or `EKR_STORE`; run `ekr seed` first |
