//! Adversary pass 2 on p1-12-replay. Helpers copied verbatim from pass 1 (itself from
//! `replay_forgery.rs`); the cases attack the corrected ordering of the named commit checks.
#![allow(dead_code)]
use ekr_core::*;
use ekr_graph::*;
use ekr_kernel::*;
...
/// Like [`with_forged_receipt`], but `result_hash` is left as the genuine one when `rehash` is
/// false, so the receipt's address of its own result is the lie.
fn forged_receipt_raw(
    history: &RetainedHistory,
    at: usize,
    rehash: bool,
    lie: impl FnOnce(&mut CommitReceiptV1),
) -> RetainedHistory {
    let mut forged = history.clone();
    let old = forged.occurrences[at].event.record_hash;
    let held = forged.objects.get(&old).unwrap().clone();
    let mut receipt = CommitReceiptV1::from_bytes(&held.bytes).unwrap();
    lie(&mut receipt);
    if rehash {
        receipt.result_hash = ContentHash::of(&receipt.result);
...
fn set_payload(history: &mut RetainedHistory, at: usize, n: Option<u64>, k: Option<ContentHash>) {
    let RevisionPayload::RevisionCommitted {
        number,
        knowledge_root,
        ..
    } = &mut history.occurrences[at].event.payload
    else {
        unreachable!()
    };
    if let Some(n) = n {
...
fn genuine_root(source: &Source) -> Root {
    let at = commit_of(&source.history, source.committed);
    let hash = source.history.occurrences[at].event.record_hash;
    CommitReceiptV1::from_bytes(&source.history.objects[&hash].bytes)
        .unwrap()
        .result
}
fn commit_result_disagrees() -> StoreError {
    StoreError::Document("commit-result-disagrees".into())
}

/// The correction narrowed the generic `require` to `record.result == root && result_hash`: every
/// root field that is *not* revision or knowledge root, lied consistently (result_hash recomputed),
/// must still be refused, and by the generic code.
#[test]
fn every_other_root_field_lied_consistently_is_still_refused_as_commit_result_disagrees() {
    let source = source();
    let lie = ContentHash::of_bytes(b"forged field");
    type Lie = (&'static str, fn(&mut Root, ContentHash));
    let lies: [Lie; 5] = [
        ("parent", |r, h| r.parent = Some(h)),
        ("ontology_root", |r, h| r.ontology_root = h),
        ("evidence_root", |r, h| r.evidence_root = h),
        ("agent_root", |r, h| r.agent_root = h),
        ("transaction", |r, h| r.transaction = h),
    ];
    for (name, apply) in lies {
        let (forged, _) = with_forged_receipt(&source, |root| apply(root, lie));
        eprintln!("field {name}");
        refused_on_both_providers(&forged, &commit_result_disagrees());
    }
}

/// A receipt whose result is genuine but whose `result_hash` is not its address.
#[test]
fn a_receipt_whose_result_hash_alone_lies_is_refused_as_commit_result_disagrees() {
    let source = source();
    let mut history = source.history.clone();
    let at = commit_of(&history, source.committed);
    history.occurrences.truncate(at + 1);
    let forged = forged_receipt_raw(&history, at, false, |r| {
        r.result_hash = ContentHash::of_bytes(b"forged result hash");
    });
    refused_on_both_providers(&forged, &commit_result_disagrees());
}

/// The payload is honest; only the receipt claims revision 2. The named refusal must fire from
/// the receipt carrier too, naming the receipt's claim.
#[test]
fn a_receipt_only_revision_lie_is_refused_out_of_order_naming_the_receipt_claim() {
    let source = source();
    let (forged, _) = with_forged_receipt(&source, |root| root.revision = RevisionNumber::new(2));
...
/// The payload is honest; only the receipt publishes an unreached knowledge root.
#[test]
fn a_receipt_only_knowledge_root_lie_is_refused_naming_the_receipt_claim() {
    let source = source();
    let folded = genuine_root(&source).knowledge_root;
    let published = ContentHash::of_bytes(b"receipt-only root");
    let (forged, _) = with_forged_receipt(&source, |root| root.knowledge_root = published);
    refused_on_both_providers(
        &forged,
...
/// Several fields lie at once, on both carriers: revision, knowledge root and an unnamed field.
/// The revision refusal is checked first, and a payload claim is reported before a receipt one.
#[test]
fn several_lies_at_once_report_the_revision_and_the_payload_claim_first() {
    let source = source();
    ...
    let (mut forged, at) = with_forged_receipt(&source, |root| {
        ...
        root.agent_root = published;
    });
    set_payload(&mut forged, at, Some(3), Some(published));
    refused_on_both_providers(
        &forged,
        &StoreError::RevisionOutOfOrder {
            expected: RevisionNumber::new(1),
            found: RevisionNumber::new(3),
        },
    );
    // Revision honest on both carriers, knowledge root lying differently on each.
    let receipt_claim = ContentHash::of_bytes(b"receipt claim");
    let (mut forged, at) = with_forged_receipt(&source, |root| {
        root.knowledge_root = receipt_claim;
        root.evidence_root = receipt_claim;
    });
    set_payload(&mut forged, at, None, Some(published));
    refused_on_both_providers(
        &forged,
        &StoreError::KnowledgeRootDisagrees {
            revision: RevisionNumber::new(1),
            published,
            folded: genuine_root(&source).knowledge_root,
        },
    );
}

/// Boundaries on the claimed number: backwards onto the seed, and the maximum.
#[test]
fn a_commit_claiming_the_seed_or_the_maximum_revision_is_refused_out_of_order() {
    let source = source();
    for claim in [0_u64, u64::MAX] {
        let (mut forged, at) =
            with_forged_receipt(&source, |root| root.revision = RevisionNumber::new(claim));
        set_payload(&mut forged, at, Some(claim), None);
        refused_on_both_providers(
            &forged,
            &StoreError::RevisionOutOfOrder {
                expected: RevisionNumber::new(1),
                found: RevisionNumber::new(claim),
            },
        );
    }
}

/// A lineage with two genuine commits; the second one's carriers both claim revision 1 again.
/// The named refusal must fire at a position other than the first commit.
#[test]
fn a_second_commit_reclaiming_revision_one_is_refused_out_of_order() {
    let directory = tempfile::tempdir().unwrap();
    let seed = fixture();
    let kernel = open(directory.path(), false);
    kernel
    ...
    let (two, _) = proposal(&seed);
    kernel
        .propose(&encode(&two), context().operator, || {
            Timestamp::from_millis(50)
        })
        .unwrap();
    assert!(matches!(
        kernel
            .validate(two.id, RevisionNumber::new(1), || Timestamp::from_millis(
                60
            ))
            .unwrap(),
        ValidationCommandResult::Validated(_)
    ...
    assert!(matches!(
        kernel
            .commit(two.id, context().operator, || Timestamp::from_millis(70))
            .unwrap(),
        CommitCommandResult::Committed(_)
    ));
    drop(kernel);
    let history = captured_history(directory.path());
    let at = commit_of(&history, two.id);
    assert_eq!(at + 1, history.occurrences.len());
    let mut forged = forged_receipt_raw(&history, at, true, |r| {
        r.result.revision = RevisionNumber::new(1);
    });
    set_payload(&mut forged, at, Some(1), None);
    refused_on_both_providers(
        &forged,
        &StoreError::RevisionOutOfOrder {
            expected: RevisionNumber::new(2),
            found: RevisionNumber::new(1),
        },
    );
}

/// `Runtime::read(Some(n))` goes through `history_at`, which truncates on the *payload* number.
/// A selected read of a forged lineage must carry the same named refusal, not a not-found.
#[test]
fn selected_reads_of_a_forged_lineage_carry_the_named_refusal() {
    let source = source();
    let (mut skipped, at) =
        with_forged_receipt(&source, |root| root.revision = RevisionNumber::new(2));
    set_payload(&mut skipped, at, Some(2), None);
    let out_of_order = StoreError::RevisionOutOfOrder {
        expected: RevisionNumber::new(1),
        found: RevisionNumber::new(2),
    };
    let published = ContentHash::of_bytes(b"forged knowledge root");
    let (mut unreached, at) = with_forged_receipt(&source, |root| root.knowledge_root = published);
    set_payload(&mut unreached, at, None, Some(published));
    let disagrees = StoreError::KnowledgeRootDisagrees {
        revision: RevisionNumber::new(1),
        published,
        folded: genuine_root(&source).knowledge_root,
    };
    for (forged, expected, selected) in [
        (&skipped, &out_of_order, 1_u64),
        (&skipped, &out_of_order, 2),
        (&unreached, &disagrees, 1),
    ] {
        for file in [false, true] {
            let directory = tempfile::tempdir().unwrap();
            install_history(directory.path(), file, forged);
            let before = physical(directory.path(), file);
            let reopened = open(directory.path(), file);
            let got = reopened.read(Some(RevisionNumber::new(selected)));
            assert!(
                matches!(got, Err(CommitError::Store(ref e)) if e == expected),
                "read(Some({selected})) file={file}: {:?}",
                got.err()
            );
            assert_eq!(physical(directory.path(), file), before, "file={file}");
        }
    }
}
