...
| same | every_event_the_crate_writes_carries_the_fields_the_domain_declares | Read actual provider metadata bodies for ObjectStored without bytes, verify the exact atomic blob binding and byte_len/content_hash separately, and retain retention-raise coverage. A dummy absent event is not evidence. |
...
| kernel/store | seed2_returns_its_original_result_after_head_advances | Complete Seeded and SeedResult stay consistent; retry after another commit returns its own Root0 with identical occurrence, context, timestamp and payload address. |
...
The 30 generated scenarios remain required; authored scenarios add semantics they cannot express. A fixture manifest may contain scenario-id -> concrete synthetic document/setup facts, never expected result objects to return. Stage only exact admitted relative regular-file paths under a temporary allowed fixture root; reject absolute/traversal/symlink escapes. Seed scenarios get their actual empty/already-seeded condition. Transaction/snapshot scenarios reach revision1 by real seed+validated commit; Explain's exact generated identity is genuinely present or absent.
